...
class Product extends BaseEntity
{
    ...
    {
        $request = ApiRequest::get(BitrixMethod::productGet->value)
            ->setQuery([
                'id' => $id
            ]);
    ...
    {
        $request = ApiRequest::get(BitrixMethod::productFields->value);
        return $this->send($request);
    }
    public function list(int $start = 1, array $select = ['*', 'PROPERTY_*'], array $filter = [], array $order = []): Response
    {
        $request = ApiRequest::post(BitrixMethod::productList->value)
            ->setBody([
                'order' => $order,
                'filter' => $filter,
                'select' => $select,
